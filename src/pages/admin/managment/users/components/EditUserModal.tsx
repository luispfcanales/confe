...
export const EditUserModal: React.FC<EditUserModalProps> = ({
  ...
  // Verificar si el rol seleccionado es "INVESTIGADOR"
  ...
  useEffect(() => {
    if (formData.faculty_id) {
      const filtered = academicDepartments.filter(
        dept => dept.faculty_id === formData.faculty_id
      );
      setFilteredDepartments(filtered);
      
      // Limpiar departamento si no pertenece a la nueva facultad
      if (formData.academic_department_id) {
        const currentDept = academicDepartments.find(
          dept => dept.ID === formData.academic_department_id
        );
        if (currentDept && currentDept.faculty_id !== formData.faculty_id) {
  ...
  }, [formData.faculty_id, academicDepartments]);
  ...
  const loadFormData = () => {
    if (user) {
      setFormData({
        ...
        is_internal: user.is_internal ?? false,
        // Campos adicionales para investigador
        researcher_type_id: user.researcher_type_id || '',
        academic_grade_id: user.academic_grade_id || '',
        participation_type_id: user.participation_type_id || '',
  ...
  const loadResearcherData = async () => {
    try {
      const [
        researcherTypesResponse,
        academicGradesResponse,
        participationTypesResponse,
        facultiesResponse,
        departmentsResponse
      ] = await Promise.all([
        fetch(`${API_URL}/api/investigator-types/all`),
        fetch(`${API_URL}/api/academic-grades`),
        fetch(`${API_URL}/api/general/participation-types`),
        fetch(`${API_URL}/api/general/faculties`),
        fetch(`${API_URL}/api/general/departments`)
      ]);
      ...
      const [
        ...
        researcherTypesResponse.json(),
        academicGradesResponse.json(),
        participationTypesResponse.json(),
        facultiesResponse.json(),
        departmentsResponse.json()
      ]);

      setResearcherTypes(researcherTypesData.data || researcherTypesData);
      setAcademicGrades(academicGradesData.data || academicGradesData);
      setParticipationTypes(participationTypesData.data || participationTypesData);
      setFaculties(facultiesData.data || facultiesData);
      setAcademicDepartments(departmentsData.data || departmentsData);
    } catch (error) {
          ...
          <div className="space-y-4">
            ...
            {isResearcherRole && (
              <div className="space-y-4 p-4 bg-blue-50 rounded-lg border border-blue-200">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <Label htmlFor="researcher_type_id">Tipo de investigador *</Label>
                    <Select
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="academic_grade_id">Grado Académico *</Label>
                    <Select
                ...
                <div className="space-y-2">
                  <Label htmlFor="participation_type_id">Tipo de Participación *</Label>
                  <Select
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="academic_department_id">Departamento académico *</Label>
                    <Select
